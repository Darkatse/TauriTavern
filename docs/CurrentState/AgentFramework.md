...
- OpenAI Chat Completions 与 Custom OpenAI Responses 支持可选流式工具参数；完整响应仍走原执行路径，前台 root / handoff write 投影到 chat，Timeline 独立展示 write/patch。
...
- `run.stream` 控制单个 invocation 的流式策略，缺失时默认 `false`；显式 `options.stream` 覆盖整次 run。`true` 仅支持已接入的 OpenAI Chat-compatible 与 Custom OpenAI Responses route，其他 route fail-fast。
