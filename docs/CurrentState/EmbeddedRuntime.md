...
chat adapter 在对应事件的末位 listener 中查询 live marker，并在下一个 `requestAnimationFrame` 无条件撤销。live query 能跟随 JSR 默认宏对 `.mes_text` 的整体重建；查询结果保存在一个短生命周期 Set 中，因此从查询完成到 rAF 之间新插入的节点不会被误解罩。adapter 不探测 iframe、wrapper、`hidden!` 或 `xb-*` 私有状态，也不等待 timer；未被 renderer 接管的源码因此恢复普通可见 fallback。
