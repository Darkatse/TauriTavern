...
- LAN Sync 状态：`default-user/user/lan-sync/`
  - LAN server 配置：`server-settings.json`（由旧 `config.json.v2_port` 一次迁移）
...
客户端策略（见 `src-tauri/src/infrastructure/tt_sync/push.rs`、`src-tauri/src/infrastructure/tt_sync/pull.rs`、`src-tauri/src/infrastructure/sync/lan/pull.rs`）：
