...
当前 Agent Gateway provider-specific 规则必须收敛在 `crates/tt-application/src/services/agent_model_gateway/providers/*` 与对应的 encode/decode/schema/provider_state 模块内，不能扩散到 runtime tool loop。
