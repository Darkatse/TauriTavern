...
截至 2026-05-02，当前 registry 开放 chat / world info / skill / workspace 四类十个内建工具。Registry 只产 canonical `AgentToolSpec`，provider-facing schema/alias 由 gateway 边界转换：
...
当前没有 MCP、shell、extension bridge、profile routing 或审批工具。
...
- Read-only。
- 返回已安装 Skill 的索引摘要。
- 当前尚未接入 profile visible/deny policy，因此结果是全局安装索引；profile policy 落地后必须在 tool dispatcher 层收窄可见范围。
- 不读取 `SKILL.md` 全文。
...
- 只能读取 UTF-8 文本；二进制、缺失文件、非法路径、symlink escape、不可见 Skill 或超预算读取都是 recoverable tool error，除非 repository 内部 IO / index 损坏等宿主级问题需要 fail-fast。
...
下一步新增 MCP 或 extension bridge 工具时，应复用这一套 registry/dispatcher/result/error 语义，而不是新建旁路。Skill policy 扩展也必须留在现有 `skill.list` / `skill.read` dispatcher 与 profile resolver 之间，不能新建第二套 Skill 读取入口。
