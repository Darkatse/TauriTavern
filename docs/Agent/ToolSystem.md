...
| `agent.await` | `agent_await` | 查询或收集当前 invocation 创建的 delegated task 结果 | 已落地 |
...
当前没有 MCP、shell、extension bridge、profile routing、Plan Mode runtime、真正后台并发、`agent.handoff` 或审批工具。
...
- 子任务不会立即并发后台执行；当前由后续 `agent.await` 按需驱动。

`agent.await`

- Read-only/control。
- 查询或收集当前 invocation 自己创建的 delegated tasks。
- 当前 queued task 会在 `agent.await` 内被运行到 terminal status；这不是最终的后台并发模型。
