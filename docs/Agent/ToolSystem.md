...
截至 2026-05-02，当前 registry 开放 chat / world info / workspace 三类八个内建工具。Registry 只产 canonical `AgentToolSpec`，provider-facing schema/alias 由 gateway 边界转换：
...
当前没有 `skill.list`、`skill.read`、MCP、shell、extension bridge 或审批工具。

### 7.1 后续内置工具候选

后续优先补齐 Skill-like 资源与策略工具：
...
- chat search/read_messages、worldinfo read_activated、workspace list/read/write/apply_patch/finish。
...
下一步新增 `skill.list`、`skill.read`、MCP 或 extension bridge 工具时，应复用这一套 registry/dispatcher/result/error 语义，而不是新建旁路。
