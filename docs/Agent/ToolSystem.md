...
截至 2026-05-04，当前 registry 开放 chat / world info / skill / workspace 四类十个内建工具。Registry 只产 canonical `AgentToolSpec`，provider-facing schema/alias 由 gateway 边界转换。Profile 会在运行时过滤 visible tool list，并可替换 model-facing ToolSpec copy 的工具 description 与参数 description；canonical specs 不被修改。
...
| `skill.read` | `skill_read` | 只读读取已安装 Skill 内的 UTF-8 文本文件 | 已落地 |
...
这些前缀由 resolved Profile 写入 run manifest，Profile 只能收窄 root universe。`persist/` 是 chat workspace 级持久 root 的 run projection：模型在 run 中通过普通 workspace 工具写入，`finalizeCommit()` 成功后才 promote 回稳定 chat workspace；未提交、失败或取消的 run 不会写回。

工具参数会写入 `tool-args/<call-id>.json`，工具结果会写入 `tool-results/<call-id>.json`，并作为 canonical `ToolResult` part 回填下一轮模型请求。Gateway 会在 provider 边界把它转换为对应 provider 格式。工具结果不会写入 SillyTavern chat 楼层。
...
- chat search/read_messages、worldinfo read_activated、skill list/read、workspace list/read/write/apply_patch/finish。
...
下一步新增 MCP 或 extension bridge 工具时，应复用这一套 registry/dispatcher/result/error 语义，而不是新建旁路。Skill policy 已留在现有 `skill.list` / `skill.read` dispatcher 与 profile resolver 之间；后续不要新建第二套 Skill 读取入口。
