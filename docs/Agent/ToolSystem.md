...
截至 2026-04-26，当前 registry 只开放两个内建工具：
...
`workspace.write_file` 当前只允许写入以下前缀：
...
工具结果会写入 `tool-results/<call-id>.json`，并作为 OpenAI-compatible `tool` message 回填下一轮模型请求。工具结果不会写入 SillyTavern chat 楼层。

Phase 2A 当前没有 `workspace.read_file`、`workspace.apply_patch`、`chat.search`、`skill.read`、MCP 或审批工具。多阶段测试应通过多次写入阶段文件验证 loop，不应要求模型读回 workspace 文件。
...
- `workspace.apply_patch` path traversal：system failure。
- denied MCP tool：policy denied。
...
Phase 1 可以没有 model-callable tool。

Phase 2 最小可合并集：

- ToolSpec/ToolResult domain model。
- ToolRegistryService。
- workspace.read_file。
- workspace.apply_patch。
- workspace.finish。
