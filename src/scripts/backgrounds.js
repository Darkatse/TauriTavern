...
async function requireOkResponse(response, action) {
    if (response.ok) {
        return;
    }
...
function assertBackgroundFoldersPayload(data) {
    if (!isPlainObject(data)) {
        throw new Error('Invalid background folders response: expected object');
    }

    if (!Array.isArray(data.folders)) {
        throw new Error('Invalid background folders response: folders must be an array');
    }

    if (!isPlainObject(data.imageFolderMap)) {
        throw new Error('Invalid background folders response: imageFolderMap must be an object');
    }

    for (const folder of data.folders) {
        if (!isPlainObject(folder) || typeof folder.id !== 'string' || typeof folder.name !== 'string') {
            throw new Error('Invalid background folders response: folder entries require id and name');
        }
    }

    for (const folderIds of Object.values(data.imageFolderMap)) {
        if (!Array.isArray(folderIds) || !folderIds.every(id => typeof id === 'string')) {
            throw new Error('Invalid background folders response: imageFolderMap values must be string arrays');
        }
    }
}
...
async function onRenameBackgroundClick(e) {
    ...
    await requireOkResponse(response, 'Rename background');
...
export async function getBackgrounds() {
    ...
    await requireOkResponse(response, 'Load backgrounds');
...
async function loadFolders() {
    ...
    if (thumbnailUpdates.length > 0) {
        const thumbnailResponse = await fetch('/api/image-metadata/folders/set-thumbnails', {
            method: 'POST',
            headers: getRequestHeaders(),
            body: JSON.stringify({ updates: thumbnailUpdates }),
        });
        await requireOkResponse(thumbnailResponse, 'Set background folder thumbnails');
    }
...
function createFolderTileElement(folder) {
    ...
    getFolderCoverUrl(folder).then(coverUrl => {
        ...
        console.warn(`Failed to load background folder cover for "${folder.name}" (${folder.id}):`, error);
    });
...
async function updateFolderAssignments(bgFiles, folderId, isRemove) {
    ...
    await requireOkResponse(response, isRemove ? 'Unassign backgrounds from folder' : 'Assign backgrounds to folder');
...
async function onCreateFolder() {
    ...
    try {
        ...
        await requireOkResponse(response, 'Create background folder');
...
async function onRenameFolder(folderId) {
    ...
    try {
        ...
        await requireOkResponse(response, 'Rename background folder');
...
async function onDeleteFolder(folderId) {
    ...
    try {
        ...
        await requireOkResponse(response, 'Delete background folder');
...
async function onSetFolderCover(bgFile) {
    ...
    try {
        ...
        await requireOkResponse(response, 'Set background folder cover');
            ...
            if (entry.target instanceof HTMLElement && entry.isIntersecting) {
                ...
                if (parentThumbnail) {
                    ...
                    resolveImageUrl(bg, isCustom, isAnimated)
                        ...
                        .catch(error => {
                            console.error('Error resolving background image:', error);
                            clipper.style.backgroundImage = PLACEHOLDER_IMAGE;
                        });
                }
...
function closeMobileBackgroundMenus() {
    $('.bg_example.mobile-menu-open, .bg_folder_tile.mobile-menu-open').removeClass('mobile-menu-open');
}

function isInsideOpenMobileBackgroundMenu(target) {
    return target instanceof Element && Boolean(target.closest('.bg_example.mobile-menu-open, .bg_folder_tile.mobile-menu-open'));
}
...
export function initBackgrounds() {
    ...
    $(document)
        .off('click', '.bg_folder_tile:not(.bg_new_folder_tile)').on('click', '.bg_folder_tile:not(.bg_new_folder_tile)', function (e) {
        ...
        .off('click', '#bg_add_folder_button').on('click', '#bg_add_folder_button', function () {
            onCreateFolder();
        })
        .off('click', '#bg_back_to_folders').on('click', '#bg_back_to_folders', function () {
            onBackToFolders();
        })
        .off('click', '.bg_folder_tile [data-action="rename-folder"]').on('click', '.bg_folder_tile [data-action="rename-folder"]', function (e) {
            e.stopPropagation();
            const folderId = $(this).closest('.bg_folder_tile').attr('data-folder-id');
            if (folderId) onRenameFolder(folderId);
        })
        .off('click', '.bg_folder_tile [data-action="delete-folder"]').on('click', '.bg_folder_tile [data-action="delete-folder"]', function (e) {
            e.stopPropagation();
            const folderId = $(this).closest('.bg_folder_tile').attr('data-folder-id');
            if (folderId) onDeleteFolder(folderId);
        })
        .off('click', '.bg_folder_tile .mobile-only-menu-toggle').on('click', '.bg_folder_tile .mobile-only-menu-toggle', function (e) {
            e.stopPropagation();
    ...
    $(document)
        ...
        .off('focusout', '.bg_example.mobile-menu-open, .bg_folder_tile.mobile-menu-open')
