...
let tauriBackgroundsAPI = null;
if (isTauri) {
    // Dynamically import the Tauri API
    import('./tauri/backgrounds-api.js').then(module => {
        tauriBackgroundsAPI = module;
        console.log('Tauri Backgrounds API loaded');
    }).catch(error => {
        console.error('Failed to load Tauri Backgrounds API:', error);
    });
}
...
async function onRenameBackgroundClick(e) {
    ...
    if (isTauri && tauriBackgroundsAPI) {
        // Use Tauri API
        try {
            console.log(`Using Tauri backend for renameBackground: ${bgNames.oldBg} to ${bgNames.newBg}`);
            await tauriBackgroundsAPI.renameBackground(bgNames.oldBg, bgNames.newBg);
            success = true;
        } catch (error) {
            console.error(`Error renaming background from ${bgNames.oldBg} to ${bgNames.newBg} using Tauri backend, falling back to server API:`, error);
...
export async function getBackgrounds() {
    ...
    if (isTauri && tauriBackgroundsAPI) {
        ...
        try {
            ...
            // Backend now returns a simple array of filenames, which is exactly what we need
            getData = await tauriBackgroundsAPI.getAllBackgrounds();
        } catch (error) {
            console.error('Error getting backgrounds from Tauri backend, falling back to server API:', error);
            // Fall back to server API
...
async function delBackground(bg) {
    if (isTauri && tauriBackgroundsAPI) {
        ...
        try {
            ...
            await tauriBackgroundsAPI.deleteBackground(bg);
        } catch (error) {
            console.error(`Error deleting background ${bg} from Tauri backend, falling back to server API:`, error);
            // Fall back to server API
...
async function uploadBackground(formData) {
    if (isTauri && tauriBackgroundsAPI) {
        ...
        try {
            ...
            const bg = await tauriBackgroundsAPI.uploadBackground(file.name, Array.from(uint8Array));
            ...
            console.error('Error uploading background using Tauri backend, falling back to server API:', error);
            // Fall back to server API
            uploadBackgroundViaAjax(formData);
        }
    } else {
        // Use server API
        uploadBackgroundViaAjax(formData);
...
/**
 * Uploads a background to the server using Ajax
 * @param {FormData} formData
 */
function uploadBackgroundViaAjax(formData) {
    jQuery.ajax({
        type: 'POST',
