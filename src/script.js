...
async function getClientVersion() {
    try {
        ...
        if (isTauri) {
            ...
            if (!data) {
                console.log('Falling back to HTTP API for client version');
        ...
        console.log('Client version data:', data);
        CLIENT_VERSION = data.agent;
        displayVersion = `TauriTavern ${data.pkgVersion}`;
...
async function firstLoadInit() {
    try {
        ...
        if (isTauri) {
            console.log('Running in Tauri environment, initializing Tauri integration...');

            // Import the Tauri integration module dynamically
            const TauriIntegration = await import('./tauri-integration.js');

            // Initialize Tauri integration
            const initialized = await TauriIntegration.initialize();

            if (initialized) {
                console.log('Tauri integration initialized successfully');
                // Get the token from Tauri integration
                token = TauriIntegration.getToken();
            } else {
                console.error('Failed to initialize Tauri integration, falling back to standard mode');
    ...
    if (isTauri) {
        try {
            ...
            if (tauriCharacter) {
                ...
                console.log('Character loaded from Tauri backend:', getData.name);
            } else {
                // If Tauri API fails, fall back to standard API
                console.warn('Tauri API failed, falling back to standard API');
            ...
            console.error('Error using Tauri API:', error);
            // Continue with standard API
            const response = await fetch('/api/characters/get', {
                method: 'POST',
                headers: getRequestHeaders(),
                body: JSON.stringify({
    ...
    if (isTauri) {
        try {
            ...
            if (tauriCharacters) {
                ...
                console.log('Characters loaded from Tauri backend:', getData.length);
            } else {
                // If Tauri API fails, fall back to standard API
                console.warn('Tauri API failed, falling back to standard API');
    ...
    try {
        ...
        if (isTauri) {
            try {
                ...
                if (tauriChat) {
                    chatData = tauriChat;
                    console.log('Chat loaded from Tauri backend:', characters[this_chid].chat);
                } else {
                    // If Tauri API fails, fall back to standard API
                    console.warn('Tauri API failed, falling back to standard API');
        ...
        if (chatData && chatData[0] !== undefined) {
...
//***************SETTINGS****************//
///////////////////////////////////////////
// Import Tauri settings adapter
import * as TauriSettingsAdapter from './tauri/settings-adapter.js';
...
export async function getSettings() {
    ...
    if (isTauri) {
        try {
            // Use Tauri settings adapter
            data = await TauriSettingsAdapter.getSettings();
            if (!data) {
                reloadLoop();
                toastr.error(t`Settings could not be loaded after multiple attempts. Please try again later.`);
                throw new Error('Error getting settings from Tauri backend');
            }
        } catch (error) {
...
export async function saveSettings(loopCounter = 0) {
    ...
    try {
        if (isTauri) {
            // Use Tauri settings adapter
            const result = await TauriSettingsAdapter.saveSettings(settingsData);
            ...
            return jQuery.ajax({
                ...
                data: JSON.stringify(settingsData, null, 4),
                beforeSend: function () { },
                cache: false,
                dataType: 'json',
                contentType: 'application/json',
