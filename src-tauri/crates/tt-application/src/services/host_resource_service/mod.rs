...
use tt_contracts::host_resource::{HostResourceRequest, HostResourceResponse, header, status};
use tt_domain::errors::DomainError;
use tt_ports::host_resource::{HostResourceAssetStore, HostResourceBinaryAsset};
...
impl HostResourceService {
    ...
    pub fn try_serve(&self, request: &HostResourceRequest<'_>) -> Option<HostResourceResponse> {
    ...
    pub fn serve(&self, request: &HostResourceRequest<'_>) -> HostResourceResponse {
...
mod tests {
    ...
    fn facade_dispatches_known_routes_and_ignores_frontend_assets() {
        ...
        let fallback = service.serve(&frontend);
        assert_eq!(fallback.status, contract::status::NOT_FOUND);
        assert!(
            header_value(&fallback, header::TAURITAVERN_TRACE_ID)
                .is_some_and(|value| value.starts_with("hr-"))
        );
    }
