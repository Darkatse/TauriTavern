...
const TTS_SOURCE: &str = "SillyTavern-TTS";
...
pub(super) async fn generate(
    ...
    let audio_content_type = content_type(&format);
    ...
    let response = send_with_retry("MiniMax TTS request", || {
        client
            ...
            .header("MM-API-Source", TTS_SOURCE)
            .json(&payload)
    })
    .await?;
    if !response.status().is_success() {
        return upstream_error_response(response, "MiniMax TTS request failed").await;
    }
    ...
    if let Some(message) = parse_base_response_error(&payload) {
        ...
        .filter(|value| !value.is_empty())
    {
        let audio = match decode_hex_audio(hex_audio) {
            Ok(audio) => audio,
            Err(message) => return Ok(error_response(502, message)),
        };
...
async fn upstream_error_response(
    ...
    Ok(error_response(
        status,
        parse_upstream_error(&bytes, fallback),
    ))
}

fn error_response(status: u16, message: impl Into<String>) -> TtsRouteResponse {
...
fn parse_upstream_error(body: &[u8], fallback: &str) -> String {
    if let Ok(payload) = serde_json::from_slice::<Value>(body)
        && let Some(message) = parse_json_error(&payload)
    {
        return message;
    }
...
fn parse_base_response_error(payload: &Value) -> Option<String> {
...
fn parse_json_error(payload: &Value) -> Option<String> {
    parse_base_response_error(payload).or_else(|| parse_json_error_message(payload))
}
...
mod tests {
    ...
    fn parses_minimax_base_resp_error() {
        assert_eq!(
            parse_base_response_error(&json!({
                "base_resp": { "status_code": 2001, "status_msg": "invalid voice" }
            }))
            .as_deref(),
            Some("API Error: invalid voice")
        );
    }

    #[test]
    fn parses_minimax_upstream_error() {
        assert_eq!(
    ...
    fn decodes_minimax_hex_audio() {
        assert_eq!(decode_hex_audio("0x0001ff").unwrap(), [0, 1, 255]);
        assert_eq!(
    ...
    #[tokio::test]
    async fn sends_expected_payload_and_decodes_hex_audio() {
        ...
        assert_eq!(response.body, [0, 1, 255]);
    ...
    #[tokio::test]
    async fn returns_json_error_for_base_resp_failure() {
        let body =
            br#"{"base_resp":{"status_code":2001,"status_msg":"\u97f3\u8272\u4e0d\u5b58\u5728"}}"#
                .to_vec();
        let (api_host, server) = spawn_one_response(200, "application/json", body).await;
