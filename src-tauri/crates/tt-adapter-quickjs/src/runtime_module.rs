...
//! 世界书与变量快照）经 `Ctx::store_userdata` 传入，由 `ModuleDef::evaluate`
//! 在模块求值时构建并导出。
...
impl ModuleDef for RuntimeV1Module {
    ...
    fn evaluate<'js>(ctx: &Ctx<'js>, exports: &Exports<'js>) -> Result<()> {
        let (overlay, world_info, variables) = match ctx.userdata::<RuntimeV1State>() {
            Some(state) => (
                state.overlay.clone(),
                state.world_info.clone(),
        ...
        let world_info_obj = crate::api::world_info::build_world_info_object(ctx, world_info)?;
        let variables_obj = crate::api::variables::build_variables_object(ctx, variables)?;

        let context = Object::new(ctx.clone())?;
        context.set("worldInfo", world_info_obj)?;
        context.set("variables", variables_obj)?;
