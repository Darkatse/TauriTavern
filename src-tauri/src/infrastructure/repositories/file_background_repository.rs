...
impl BackgroundRepository for FileBackgroundRepository {
    async fn get_all_backgrounds(&self) -> Result<Vec<Background>, DomainError> {
        logger::debug(&format!("FileBackgroundRepository: Getting all backgrounds from {:?}", self.backgrounds_dir));
        
        // Ensure the directory exists
        if !self.backgrounds_dir.exists() {
            fs::create_dir_all(&self.backgrounds_dir).await.map_err(|e| {
                logger::error(&format!("Failed to create backgrounds directory: {}", e));
                DomainError::InternalError(format!("Failed to create backgrounds directory: {}", e))
            })?;
            return Ok(Vec::new());
        }
    ...
    async fn delete_background(&self, filename: &str) -> Result<(), DomainError> {
        logger::debug(&format!("FileBackgroundRepository: Deleting background: {}", filename));
        
        let file_path = self.get_full_path(filename);
        
        // Check if the file exists
        if !file_path.exists() {
            return Err(DomainError::NotFound(format!("Background not found: {}", filename)));
        }
    ...
    async fn rename_background(&self, old_filename: &str, new_filename: &str) -> Result<(), DomainError> {
        logger::debug(&format!("FileBackgroundRepository: Renaming background from '{}' to '{}'", old_filename, new_filename));
        ...
        if !old_path.exists() {
            return Err(DomainError::NotFound(format!("Background not found: {}", old_filename)));
        }
        
        // Check if the destination file already exists
        if new_path.exists() {
            return Err(DomainError::InvalidData(format!("Background already exists: {}", new_filename)));
        }
    ...
    async fn upload_background(&self, filename: &str, data: &[u8]) -> Result<String, DomainError> {
        logger::debug(&format!("FileBackgroundRepository: Uploading background: {}", filename));
