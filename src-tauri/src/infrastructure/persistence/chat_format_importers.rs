...
use crate::domain::models::chat::{Chat, ChatMessage, MessageExtra, humanized_date, message_date_format};
...
pub fn import_from_sillytavern(data: &Value) -> Result<Chat, DomainError> {
    ...
    let character_name = data["character_name"].as_str().unwrap_or("Character").to_string();
    let create_date = data["create_date"].as_str().unwrap_or(&humanized_date(Utc::now())).to_string();
