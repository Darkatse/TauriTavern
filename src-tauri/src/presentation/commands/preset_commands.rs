...
use crate::application::dto::preset_dto::{
    SavePresetDto, DeletePresetDto, RestorePresetDto,
    SavePresetResponseDto, RestorePresetResponseDto,
    SaveOpenAIPresetDto, DeleteOpenAIPresetDto, DeleteOpenAIPresetResponseDto
};
use crate::domain::models::preset::{PresetType, sanitize_filename};
...
pub async fn save_preset(
    ...
    logger::debug(&format!("Command: save_preset, name: {}, api_id: {}", dto.name, dto.api_id));

    // Validate input
    if dto.name.trim().is_empty() {
        logger::warn("Preset name is empty");
        return Err(CommandError::BadRequest("Preset name cannot be empty".to_string()));
    }

    if dto.preset.is_null() {
        logger::warn("Preset data is null");
        return Err(CommandError::BadRequest("Preset data cannot be null".to_string()));
    }

    // Create preset from DTO
    let preset = app_state.preset_service.create_preset(
        dto.name.clone(),
        &dto.api_id,
        dto.preset,
    ).map_err(|e| {
    ...
    // Save preset
    app_state.preset_service.save_preset(&preset).await.map_err(|e| {
...
pub async fn delete_preset(
    ...
    logger::debug(&format!("Command: delete_preset, name: {}, api_id: {}", dto.name, dto.api_id));
    ...
    // Delete preset
    app_state.preset_service.delete_preset(&dto.name, &preset_type).await.map_err(|e| {
...
pub async fn restore_preset(
    ...
    logger::debug(&format!("Command: restore_preset, name: {}, api_id: {}", dto.name, dto.api_id));
    ...
    match app_state.preset_service.restore_default_preset(&dto.name, &preset_type).await {
        Ok(Some(default_preset)) => {
            logger::info(&format!("Default preset found for restoration: {}", dto.name));
...
pub async fn save_openai_preset(
    ...
    logger::info(&format!("OpenAI preset saved successfully: {}", preset.name));
    Ok(SavePresetResponseDto::new(preset.name))
}
...
pub async fn delete_openai_preset(
    ...
    logger::debug(&format!("Command: delete_openai_preset, name: {}", dto.name));
    ...
    // Delete preset
    match app_state.preset_service.delete_preset(&dto.name, &PresetType::OpenAI).await {
        Ok(()) => {
...
pub async fn list_presets(
    ...
    // List presets
    let presets = app_state.preset_service.list_presets(&preset_type).await.map_err(|e| {
        logger::error(&format!("Failed to list presets: {}", e));
        CommandError::from(e)
    })?;

    logger::debug(&format!("Found {} presets of type {}", presets.len(), api_id));
    Ok(presets)
}
...
pub async fn preset_exists(
    ...
    logger::debug(&format!("Command: preset_exists, name: {}, api_id: {}", name, api_id));
    ...
    let exists = app_state.preset_service.preset_exists(&name, &preset_type).await.map_err(|e| {
...
pub async fn get_preset(
    ...
    logger::debug(&format!("Command: get_preset, name: {}, api_id: {}", name, api_id));
    ...
    // Get preset
    let preset = app_state.preset_service.get_preset(&name, &preset_type).await.map_err(|e| {
