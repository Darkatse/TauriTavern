...
#[derive(Debug, Clone)]
pub(crate) struct UploadUserImageInput {
    pub(crate) image_base64: String,
    pub(crate) format: String,
    pub(crate) filename: Option<String>,
    pub(crate) ch_name: Option<String>,
}

#[derive(Debug, Clone)]
pub(crate) struct ListUserImagesInput {
...
#[derive(Clone)]
pub(crate) struct UserMediaService {
...
impl UserMediaService {
    pub(crate) fn new(store: Arc<dyn UserMediaStore>) -> Self {
        Self { store }
    }

    pub(crate) async fn upload_user_image(
        &self,
    ...
    pub(crate) async fn list_user_images(
        &self,
    ...
    pub(crate) async fn list_user_image_folders(&self) -> Result<Vec<String>, DomainError> {
        let mut folders = self.store.list_folders().await.map_err(DomainError::from)?;
        folders.sort();
        Ok(folders)
    }

    pub(crate) async fn delete_user_image(&self, path_or_url: &str) -> Result<(), DomainError> {
