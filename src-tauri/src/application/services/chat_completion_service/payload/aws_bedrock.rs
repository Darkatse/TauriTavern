...
pub(super) fn build(payload: Map<String, Value>) -> Result<(String, Value), ApplicationError> {
    ...
    match detect_provider(&model_id) {
        ...
        BedrockProvider::Other(provider) => Err(ApplicationError::ValidationError(format!(
            "AWS Bedrock provider `{provider}` is not yet wired up by TauriTavern. Currently supported: Anthropic, Amazon Nova, Meta Llama, Mistral, DeepSeek. Other providers (Cohere, AI21) will be added in follow-up releases."
        ))),
    }
}
...
mod tests {
    ...
    use super::{
        ...
        format_deepseek_r1_prompt, format_llama3_chat_prompt, is_deepseek_text_completion,
    ...
    fn detect_provider_surfaces_unsupported_providers_for_clear_error() {
        // Cohere and AI21 still surface as Other(<provider>) until follow-up
        // PRs wire builders.
        assert_eq!(
