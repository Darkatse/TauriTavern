...
pub(super) fn build(payload: Map<String, Value>) -> Result<(String, Value), ApplicationError> {
    ...
    match detect_provider(&model_id) {
        ...
        BedrockProvider::Other(provider) => Err(ApplicationError::ValidationError(format!(
            "AWS Bedrock provider `{provider}` is not yet wired up by TauriTavern. Currently supported: Anthropic, Amazon Nova, Meta Llama. Other providers (Mistral, Cohere, AI21, DeepSeek) will be added in follow-up releases."
        ))),
    }
}
...
mod tests {
    ...
    fn detect_provider_surfaces_unsupported_providers_for_clear_error() {
        // Mistral, Cohere, AI21, DeepSeek all surface as Other(<provider>)
        // until follow-up PRs wire builders.
        assert_eq!(
