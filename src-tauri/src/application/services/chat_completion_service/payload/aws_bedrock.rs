...
/// Build a DeepSeek invoke body. Bedrock hosts two DeepSeek schemas:
///
...
///    do not pin the shape. We treat the response defensively in
///    `deepseek_response_to_claude_shape` by probing both `choices[].text` and
///    `choices[].message.content`.
///
/// Dispatch is driven by the Bedrock model id: anything containing `r1` is the
/// reasoning model and takes the text-completion path with the R1 template;
/// everything else (`v3`, `v3-1`, `v3.2`, ...) uses the chat-completion path.
fn build_deepseek(
    payload: Map<String, Value>,
    model_id: &str,
) -> Result<(String, Value), ApplicationError> {
    ...
    if is_deepseek_text_completion(model_id) {
        return Ok((endpoint_path, build_deepseek_text_completion_body(payload)));
    }

    Ok((endpoint_path, build_deepseek_chat_completion_body(payload)))
}

fn is_deepseek_text_completion(model_id: &str) -> bool {
...
fn build_deepseek_text_completion_body(payload: Map<String, Value>) -> Value {
    let (system_text, conversation) = flatten_openai_messages(payload.get("messages"));
    let prompt = format_deepseek_r1_prompt(system_text.as_deref(), &conversation);
...
fn build_deepseek_chat_completion_body(payload: Map<String, Value>) -> Value {
    let messages = passthrough_chat_messages(payload.get("messages"));
...
/// on `/model/{id}/invoke`; the schema is **not** OpenAI-shaped — the latest
/// user message is split out into a dedicated `message` field while prior
/// turns are projected into a `chat_history` array of `{role, message}`
/// entries.
///
...
/// Command R API). Legacy `cohere.command-text-v14` / `command-light-text-v14`
/// expose a different `prompt`-based schema with `generations[].text`; they
/// will return a Bedrock validation error today and are tracked for a
/// follow-up. The frontend already gates `embed*` / `rerank*` out of the
/// chat-completion picker.
fn build_cohere(
    payload: Map<String, Value>,
    model_id: &str,
) -> Result<(String, Value), ApplicationError> {
    ...
    let chat_history = build_cohere_chat_history(&conversation);
...
fn build_cohere_chat_history(turns: &[FlatMessage]) -> Vec<Value> {
    turns
        .iter()
        .filter_map(|turn| {
...
pub(super) fn format_deepseek_r1_prompt(
    system: Option<&str>,
    turns: &[FlatMessage],
) -> String {
...
mod tests {
    ...
    use super::{
        BedrockProvider, FlatMessage, build, build_cohere_chat_history, detect_provider,
        flatten_openai_messages, format_deepseek_r1_prompt, format_llama3_chat_prompt,
        is_deepseek_text_completion, normalize_bedrock_model_id,
    };
    ...
    #[test]
    fn is_deepseek_text_completion_only_matches_r1_variants() {
        assert!(is_deepseek_text_completion("deepseek.r1-v1:0"));
        assert!(is_deepseek_text_completion("us.deepseek.r1-v1:0"));
        assert!(!is_deepseek_text_completion("deepseek.v3-v1:0"));
        assert!(!is_deepseek_text_completion("deepseek.v3.2"));
    }
    ...
    #[test]
    fn build_cohere_chat_history_drops_empty_messages_and_marks_roles() {
        let turns = vec![
            FlatMessage {
                role: "user".to_string(),
        ...
        let history = build_cohere_chat_history(&turns);
        assert_eq!(history.len(), 2);
        assert_eq!(history[0]["role"], "USER");
    ...
    #[test]
    fn format_deepseek_r1_prompt_handles_multi_turn_history() {
        let turns = vec![
            FlatMessage {
                role: "user".to_string(),
        ...
        let prompt = format_deepseek_r1_prompt(Some("be terse"), &turns);
